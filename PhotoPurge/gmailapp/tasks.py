...
def delete_emails_task(self, user_id, email, category):
	try:
		# task_status variable creates a new object of TaskStatus class or if it already existing
		# it fetches its data by task_id and user_id and created is boolean variable
		# which returns True or False for created or not created
	...
	try:		
		...
		# query to search emails in the category selected by user
		query = f"category:{category.split('_')[1].lower()}"
		deleted_count = 0
		page_token = None

		# loop to fetch all emails in the selected category
		while True:
			try:
